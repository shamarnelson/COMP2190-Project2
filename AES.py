...
#Write a function that accepts a message as a String and returns a list with the ASCII value for each character in the message.
def strToASCI(message): 
    """Your Code here."""
    if message == None or len(message) == 0:
        return []
    ascii_values = []
    for m in message:
        ascii_values.append(ord(m))
    return ascii_values

# Write a function that accepts a list of ASCII values and encrypts each of them. The function then returns a String of encrypted values with a space between each value.
def encryptMessage(ASCI_mess):
    """Your Code here."""
    if ASCI_mess == None or len(ASCI_mess) == 0:
        return ""
    encrypted_values = []
    for m in ASCI_mess:
        value = str(encrypt(m))
        encrypted_values.append(value)
    string_values = " ".join(encrypted_values)
    return string_values

# Write a function that accepts a String of ASCII values and decrypts each of them. The function then returns the decrypted message as a String.
def decryptMessage(e_message): 
    """Your Code here."""
    if e_message == None or len(e_message) == 0:
        return ""
    decrypted_values = []
    for m in e_message.split(' '):
        value = chr(decrypt(int(m)))
        decrypted_values.append(value)
    string_values = "".join(decrypted_values)
    return string_values
